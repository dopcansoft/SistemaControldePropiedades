$(document).on("ready",function(){
	...
	$("#btn-imprimir").on("click", function(){
		var ventimp = window.open('inventario.imp.php?estatus=3&departamento='+$("#sl-departamento").val()+"&estadoFisico="+$("#sl-estado-fisico").val()+"&clasificacionBm="+$("#sl-clasificacion").val());
	});
	...
	$("#btn-buscar").on("click", function(){
		...
		$.ajax({
			url:'src/services/filtradobajas.php',
			type:'POST',
			data:ajax_data,
			...
			success:function(respuesta){
				...
				if(respuesta.result=="SUCCESS"){
					...
					for(var i=0;i<respuesta.data.length;i++){
						...
						$row.find("td:eq(7)").html($datos[i].valor);
						if(typeof $datos[i].imagen!='undefined' && $datos[i].imagen!=""){
							$row.find("td:eq(8)").html("<img src=\""+$datos[i].imagen+"\" style=\"width: 50px;\">");
						}						

						$row.find("td:eq(9)").html($datos[i].clasificacion.cuentaContable);
						$row.find("td:eq(10)").html($datos[i].clasificacion.cuentaDepreciacion);
						var fechaInsert = moment($datos[i].fechaInsert); 
						$row.find("td:eq(11)").html(fechaInsert.format("DD/MM/YYYY H:m:s"));
						

						$row.find("td:eq(12) a:eq(0)").attr("data-id",$datos[i].id);
						$row.find("td:eq(12) a:eq(0)").attr("data-id-empresa",$datos[i].empresa.id);
						$row.find("td:eq(12) a:eq(0)").attr("data-id-periodo",$datos[i].periodo.id);
						
						$row.find("td:eq(12) a:eq(1)").attr("data-id",$datos[i].id);
						$row.find("td:eq(12) a:eq(1)").attr("data-id-periodo",$datos[i].periodo.id);
						$row.find("td:eq(12) a:eq(1)").attr("href","bien.php?id="+$datos[i].id);

						$row.find("td:eq(12) a:eq(2)").attr("data-id",$datos[i].id);
						$row.find("td:eq(12) a:eq(2)").attr("data-id-periodo",$datos[i].periodo.id);
						
						$row.find("td:eq(12) a:eq(3)").attr("data-id",$datos[i].id);
						$row.find("td:eq(12) a:eq(3)").attr("data-id-periodo",$datos[i].periodo.id);
						
						if($("#perfil").val()==1||$("#perfil").val()==3){

						}else{
							/*$row.find("td:eq(11) a:eq(3)").remove();
							$row.find("td:eq(11) a:eq(2)").remove();*/
						}


						/*$row.find("td:eq(9) button:eq(1)").attr("data-id",$datos[i].id);
						$row.find("td:eq(9) button:eq(1)").attr("data-id-periodo",$datos[i].periodo.id);						
						$row.find("td:eq(9) button:eq(2)").attr("data-id-periodo",$datos[i].periodo.id);*/						
