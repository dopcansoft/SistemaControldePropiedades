						...
						if(count($bien->images)>0 && isset($bien->images[0]) && $bien->images[0]!=""){
							...
							<table class="table-report-fotos">
								<thead>
									...
									<tr>
										<td colspan="2"><h3><small>Folio: <strong><?=$bien->folio?><strong></small></h3></td>
									</tr>
								</thead>
								<tbody>
									<?
												...
												for($j=$i;$j<$i+2;$j++){
													if(isset($bien->images[$j+$i]) && $bien->images[$j+$i]!=""){
														if($j==$i && (!isset($bien->images[1+$i+$j]) || $bien->images[1+$i+$j]=="")){
															?>
															<td colspan="2" class="report-content-img"><img class="report-img-single" src="<?=$settings->prop("system.url").$bien->images[$j+$i]?>"></td>
															<?		
														}else{
															?>
															<td class="report-content-img"><img class="report-img-single" src="<?=$settings->prop("system.url").$bien->images[$j+$i]?>"></td>
															<?
														}																	
													}															
												}
