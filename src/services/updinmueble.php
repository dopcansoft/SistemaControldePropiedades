	...
	if($dataInput->validSqueme(array(
		...
		try{
			...
			if(isset($db)){
				...
				$db->beginTransaction();
				if($successFile){
					if($inmueble->update($db, $queries, $log)){
						if($inmueble->deleteImgs($db, $queries, $log)){
							if($inmueble->insertImages($images, $db, $queries, $log)){
								$db->commit();
								$response["result"] = "SUCCESS";
								$response["desc"] = "Se actualizo satisfactoriamente el inmueble";
						...
						$db->rollBack();
						$response["result"] = "FAIL";
						$response["desc"] = "No se ha podido actualizar el Inmueble";
					}	
				}else{
					$db->rollBack();
